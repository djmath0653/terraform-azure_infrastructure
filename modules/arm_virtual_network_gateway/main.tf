...
module "public_ip_address" {
  ...
  location            = var.location
  name                = coalesce(each.value.public_ip.name, "${var.public_ip_name}${each.key}")
  resource_group_name = var.virtual_network_resource_group_name
...
resource "azurerm_virtual_network_gateway" "vgw" {
  ...
  dynamic "ip_configuration" {
    ...
    content {
      public_ip_address_id          = module.public_ip_address[ip_configuration.key].id
      subnet_id                     = var.subnet_id
  ...
  lifecycle {
    ignore_changes = [
      # Static reference to the first element
      ip_configuration
    ]
  }
}
